...
static const AVOption options[] = {
    ...
    { "http_host", "set custom HTTP host, can override built in default hostname", OFFSET(hostname), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
...
static int http_open_cnx_internal(URLContext *h, AVDictionary **options)
{
    ...
    /*
    //lujun
    av_dict_getall(s, *options);
    AVDictionaryEntry *de = NULL;
    de = av_dict_get(*options, "hostname", NULL, AV_DICT_IGNORE_SUFFIX);
    if (s->hostname == NULL || strlen(s->hostname) == 0){
    	if(de){
    		av_log(NULL, AV_LOG_DEBUG,"11111\n");
    		if(s->hostname)
    			av_free(s->hostname);
    		s->hostname = av_strdup(de->value);
    	}
    }else{
		if(!de){
			av_log(NULL, AV_LOG_DEBUG,"22222\n");
			av_dict_set(options, "hostname", s->hostname, AV_DICT_DONT_OVERWRITE);
			av_log(NULL, AV_LOG_DEBUG,"333333\n");
		}
    }
    av_dict_getall(s, *options);
    //lujun
